...
function handleContributorSave(event) {
    ...
    $.ajax({
        ...
        init();
        $('#contribute .message').text(
            `${contributorEmail} is now a contributor on this board`
        ).attr('class', 'message success');
    })
    .catch(function() {
        $('#contribute .message').text(
            `Cannot find user email: ${contributorEmail}`
        ).attr('class', 'message danger');
    });
}
...
function handleContributorDelete() {
    ...
    $.ajax({
        ...
        init();
        $('#contribute .message').text(
            `${email} no longer has access to board`
        ).attr('class', 'message success');
    });
}
